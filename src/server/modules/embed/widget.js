...
(function () {
  ...
  var config = {
    siteKey: siteKey,
    apiBase: (script.getAttribute('data-api-base') || 'http://localhost:3001').replace(/\/api\/?$/, '').replace(/\/$/, ''),
  ...
  var css = '\
    ...
    #nemark-chat-panel.minimized {\
      height: 60px !important;\
      transition: height 0.2s ease;\
    }\
    #nemark-chat-panel.maximized {\
      width: 100vw !important;\
      height: 100vh !important;\
      bottom: 0 !important;\
      right: 0 !important;\
  ...
  function createIframe() {
    ...
    var iframeSrc = config.apiBase + '/api/embed/frame' +
      ...
      (config.welcomeMessage ? '&welcomeMessage=' + encodeURIComponent(config.welcomeMessage) : '') +
  ...
  function handleMessage(event) {
    ...
    switch (data.type) {
      ...
      case 'WIDGET_MINIMIZE':
        ...
        try { localStorage.setItem('nemark_widget_state', panel.classList.contains('minimized') ? 'minimized' : 'normal'); } catch (e) { }
        break;

      case 'WIDGET_MAXIMIZE':
        panel.classList.remove('minimized');
        panel.classList.toggle('maximized');
        try { localStorage.setItem('nemark_widget_state', panel.classList.contains('maximized') ? 'maximized' : 'normal'); } catch (e) { }
        break;
    }
  }
    ...
    if (window.fetch) {
      fetch(checkUrl)
        ...
        .then(function (data) {
          ...
          if (data && data.data) {
            var remoteConfig = data.data;
            if (remoteConfig.name) {
              config.title = remoteConfig.name;
            }
            if (remoteConfig.theme) {
              try {
                var theme = typeof remoteConfig.theme === 'string' ? JSON.parse(remoteConfig.theme) : remoteConfig.theme;

                if (theme.title) config.title = theme.title;
                if (theme.color) config.primaryColor = theme.color;
                if (theme.mainColor) config.primaryColor = theme.mainColor;
                if (theme.position) config.position = theme.position === 'bl' ? 'bottom-left' : 'bottom-right';
                if (typeof theme.autoOpen !== 'undefined') config.autoOpen = theme.autoOpen;
                if (theme.welcomeMessage) config.welcomeMessage = theme.welcomeMessage;

                // Update bubble color immediately if created
                // But init() hasn't run yet, so config update is enough.
              } catch (e) {
                console.warn('Nemark Chat: Invalid theme config', e);
              }
            }
          }
          init();
        ...
        .catch(function (err) {
          ...
          // Still init if just config check failed? No, security check failure usually means domain invalid.
          // But if it's just a fetch error (network), maybe fallback to defaults?
          // For now, consistent with existing logic: log and stop? 
          // Previous logic was just console.warn.
        });
    } else {
