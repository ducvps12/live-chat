...
const getFrame = (req, res) => {
  try {
    ...
    let html = fs.readFileSync(framePath, 'utf8');

    // Inject configuration
    html = html.replace('{{BACKEND_URL}}', BACKEND_PUBLIC_URL);
...
const getDemo = asyncHandler(async (req, res) => {
  ...
  if (!demoSiteKey) {
    ...
    const [rows] = await pool.execute(
      'SELECT SiteKey FROM iam_Widgets WHERE Status = 1 AND SiteKey IS NOT NULL LIMIT 1'
    );
    if (rows.length > 0) {
      demoSiteKey = rows[0].SiteKey;
    }
  }
 ...
 */
const getWorkspaceConversations = asyncHandler(async (req, res) => {
  ...
  const conversations = await embedService.listConversationsForWorkspace(
    workspaceId,
    user.UserKey,
    parseInt(limit, 10),
    parseInt(offset, 10)
 ...
 * Get workspace conversation statistics
 * Requires authentication
 */
const getWorkspaceStats = asyncHandler(async (req, res) => {
  ...
  const stats = await embedService.getWorkspaceStats(workspaceId, user.UserKey);
...
/**
 * GET /api/embed/workspaces/:workspaceId/conversations/search
 * Search conversations in a workspace
 * Query params: keyword, status, dateFrom, dateTo, limit, offset
 */
const searchConversations = asyncHandler(async (req, res) => {
  const { workspaceId } = req.params;
  const { keyword, status, dateFrom, dateTo, limit = 50, offset = 0 } = req.query;
  const user = req.user;
  ...
  const conversations = await embedService.searchConversations(
    workspaceId,
    user.UserKey,
    {
      keyword,
 ...
 * Body: { status: 1|2|3 } (1=active, 2=closed, 3=pending)
 */
const updateConversationStatus = asyncHandler(async (req, res) => {
  ...
  if (!status || ![1, 2, 3].includes(status)) {
    throw new AppError('Invalid status. Must be 1 (active), 2 (closed), or 3 (pending)', 400);
  }

  await embedService.updateConversationStatus(conversationId, status);
...
module.exports = {
  ...
  assignConversation: asyncHandler(async (req, res) => {
    const { conversationId } = req.params;
    const { agentId } = req.body; // Optional: manual assign

    // Default to current user if agentId not provided
    const targetAgentId = agentId || req.user.UserKey;

    await embedService.assignConversation(conversationId, targetAgentId);
   ...
   */
  updateVisitorContact: asyncHandler(async (req, res) => {
    const { conversationId } = req.params;
    ...
    if (!name && !email && !phone) {
      throw new AppError('At least one field (name, email, phone) is required', 400);
    }
    ...
    const result = await conversationRepo.updateVisitorContact(conversationId, { name, email, phone });
