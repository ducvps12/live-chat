...
export const scopeCheck = async (req: Request, _res: Response, next: NextFunction) => {
    try {
        ...
        if (!user) return next(new AppError('ChÆ°a xÃ¡c thá»±c', 401, 'UNAUTHORIZED'));
        ...
        if (user.workspaces && Array.isArray(user.workspaces)) {
            ...
            if (!membership) return next(new AppError('Báº¡n khÃ´ng thuá»™c workspace nÃ y', 403, 'FORBIDDEN_WORKSPACE'));
            ...
            if (!member || !member.workspace.isActive) return next(new AppError('Báº¡n khÃ´ng thuá»™c workspace nÃ y', 403, 'FORBIDDEN_WORKSPACE'));
        ...
        if (conversationId) {
            try {
                ...
                if (!conv) return next(new AppError('Conversation khÃ´ng tá»“n táº¡i', 404, 'CONVERSATION_NOT_FOUND'));
                const wsRole = (req as any).workspaceRole;
                if (wsRole !== 'owner' && wsRole !== 'admin' && conv.assignedTo && conv.assignedTo !== user.id) {
...
export const socketScopeCheck = async (
    ...
    if (!user) return { allowed: false, code: 'UNAUTHORIZED', message: 'ChÆ°a xÃ¡c thá»±c' };
    ...
    if (conversationId) {
        try {
            const conv = await prisma.conversation.findFirst({ where: { id: conversationId, workspaceId } });
            if (!conv) return { allowed: false, code: 'CONVERSATION_NOT_FOUND', message: 'Conversation khÃ´ng tá»“n táº¡i' };
            if (conv.assignedTo && conv.assignedTo !== user.id) return { allowed: false, code: 'FORBIDDEN_CONVERSATION', message: 'Không có quyền' };
        } catch { /* skip */ }
    }
    return { allowed: true };
};
